...
**重要**：以下情況必須立即發送 LINE 通知，不可遺漏：

**系統提示指令**：當指令中包含 `【系統提示】` 或 `此指令來自 LINE` 字樣時，任務完成後務必回報結果。
...
1. **使用腳本參數模式**：
   - ✅ **正確**：`python3 line_notifier.py '訊息內容'`
   - ❌ **避免**：`python3 -c "from line_notifier..."` (易發生轉義錯誤)
...
```bash
# 發送測試通知
python3 line_notifier.py '🧪 {agent_name} 測試訊息：系統正常運作！'

# 一般回應
python3 line_notifier.py '💬 您好！我是 {agent_name}\n已收到您的訊息並正在回應'

# 系統互動確認
python3 line_notifier.py '🤖 {agent_name} 收到指令\n正在處理您的請求...'
```
---

### 🕐 通知發送時機
- **所有用戶互動**：每次用戶發送任何訊息時必須立即發送 LINE 通知回應
- **一般對話**：問候、詢問、閒聊等所有對話都要透過 LINE 回應
- **系統互動開始**：每次用戶請求時立即發送 LINE 通知確認收到
